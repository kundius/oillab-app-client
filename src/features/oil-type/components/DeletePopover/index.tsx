...
export function DeletePopover ({
  ...
  const [deleteMutation, deleteMutationState] = schema.useOilTypeDeletePopoverMutation()
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (response.data?.oiltypeDelete.success) {
      onDelete?.()
      apollo.cache.evict({
        id: 'ROOT_QUERY',
    ...
    if (response.data?.oiltypeDelete.error) {
      await showToast({
        message: response.data.oiltypeDelete.error.message,
